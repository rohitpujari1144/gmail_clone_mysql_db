...
function TrashEmails() {
    let [trashEmails, setTrashEmails] = useState([])
    ...
    async function getAllTrashEmails() {
        try {
            let res = await axios.get(`http://localhost:10500/email/all_trash_emails?userEmail=${userEmail}`)
            if (res.data.length) {
                setTrashEmails(res.data)
            }
            else{
                setTrashEmails([])
            }
    ...
    async function emptyTrashNow() {
        if (trashEmails.length) {
            try {
                await axios.delete(`http://localhost:10500/email/delete_all_trash?userEmail=${userEmail}`)
                infoToastMessage('All trash emails deleted', 3000)
                getAllTrashEmails()
            }
            catch (error) {
                if (!error.response) {
            ...
            infoToastMessage('No trash emails found', 3000)
        }
    }
            ...
            <div style={{ marginTop: '30px' }}>
                <div className='row'>
                    ...
                    <div className='col border border-dark rounded' style={{ minHeight: '600px', marginRight: '40px' }}>
                        <div className='mt-2'>
                            <i className="fs-5 fa-solid fa-arrow-rotate-right icon" title='Refresh' onClick={() => { getAllTrashEmails() }}></i>
                        </div>
                                ...
                                trashEmails.length ? trashEmails.map((e, i) => {
                                    return (
                                        <div key={i} className="row mt-3 mb-1 p-1 unreadMail rounded-3" style={{ marginLeft: '1px', marginRight: '1px' }} >
                                            ...
                                            {/* <div className='col-1 '>
                                                <i className="fs-5 fa-solid fa-trash-arrow-up icon delete" title='Move to Inbox'></i>
                                            </div> */}
                                        </div>
                                    )
                                }) : <h5 className='text-center text-danger'>No Trash Emails</h5>
                            }
                        </div>
                    </div>
                </div>
